...
class EditsServiceTest {
    ...
    fun `Uploads apk to Edit`() {
        ...
        val apkMock = mock<Apk>()
        val fileContentMock = mock<FileContent>()
        doReturn(apksMock).`when`(editsMock).apks()
        doReturn(uploadMock).`when`(apksMock).upload(any(), eq(editId), any())
        doReturn(fileContentMock).`when`(apkLoaderMock).loadApk()
        doReturn(apkMock).`when`(uploadMock).execute()
        doReturn(mock<Properties>()).`when`(propertyLoaderMock).load(any())
    ...
    fun `Updates Track info`(){
        val tracksMock = mock<AndroidPublisher.Edits.Tracks>()
        val updateMock = mock<AndroidPublisher.Edits.Tracks.Update>()
        doReturn(mock<Properties>()).`when`(propertyLoaderMock).load(any())
    ...
    fun `Commits Track changes`(){
        val commitMock = mock<AndroidPublisher.Edits.Commit>()
        val appEditMock = mock<AppEdit>()
        doReturn(mock<Properties>()).`when`(propertyLoaderMock).load(any())
        doReturn(commitMock).`when`(editsMock).commit(any(), eq(editId))
        doReturn(appEditMock).`when`(commitMock).execute()
